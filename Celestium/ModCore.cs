...
namespace Celestium
{
    public class ModCore : ModKernel
    {
            ...
            else if (commandComps[0] == "celestium")
            {
                if (commandComps.Length > 1)
                {
                    if (commandComps[1] == "move")
                    {
                        ...
                        if (GraphicalMap.selectedHex == null || GraphicalMap.selectedHex.location == null || GraphicalMap.selectedHex.location == CelestiumGod.Settlement.location)
                        {
                            return;
                        }
        ...
        public override float hexHabitability(Hex hex, float hab)
        {
            ...
            if (CelestiumGod.TemperatureMap.TryGetValue(hex.map.grid[hex.x][hex.y], out God_Celestium.TemperatureModifier modifier))
            {
                if (modifier.IsLava)
                {
                    return -1f;
